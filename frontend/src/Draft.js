...
const AppointmentForm = ({ token }) => {
  ...
  const filterAvailableTimes = useCallback((selectedDate) => {
    const bookedTimes = appointments
      .filter(appointment => isSameDay(new Date(appointment.date_time), selectedDate))
      .map(appointment => new Date(appointment.date_time).getHours());
    
    const allTimes = Array.from({ length: 24 }, (_, index) => index); // All hours in a day
    const availableTimes = allTimes.filter(hour => !bookedTimes.includes(hour));
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const response = await fetch(`${apiProxy}/api/specialist_appointments/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        const bookedDates = data.map(appointment => new Date(appointment.date_time));
  ...
  const waitForTransaction = async (appointmentId) => {
    ...
    while (attempts < MAX_ATTEMPTS) {
        try {
            const response = await fetch(`${apiProxy}/api/payment_waiting/${appointmentId}/`, {
                method: 'GET',
                headers: {
                    Authorization: `Bearer ${token}`,
                },
        ...
        <Stack maxWidth='450px'>
            ...
            <form onSubmit={handleFormSubmit}>
              <Box display='flex' justifyContent='space-between' mt={5}>
                <DatePicker
                  ...
                  filterDate={filterDate} // Disable booked dates
                />
                <select
                  value={time}
              ...
              <Box textAlign='center'>
                <Button type='submit' variant='contained' color='primary' disabled={loading}>
                  {loading ?  `Processing ${countdown} s` : 'Book Appointment'}
                </Button>
              </Box>
            </form>
          </Box>
          ...
          <Dialog open={successDialogOpen} onClose={handleSuccessDialogClose}>
            ...
            <DialogContent>
              ...
              {transactionDetails && (
                <Stack gap={2} mt={2}>
                  <Typography variant='body1' color='success'>Amount Received: {transactionDetails.amount_received}</Typography>
                  <Typography variant='body1' color='success'>Transaction ID: {transactionDetails.transaction_id}</Typography>
                </Stack>
              )}
            </DialogContent>
            <DialogActions>
              ...
              <div>
                <Stack mt={2} gap={2}>
                  <Paper sx={{p: 2, display: 'flex', flexDirection: 'column', gap: 3}}>
                    <Typography variant='h6' color='Purple' display='inline-flex' gap={2}>
                    ...
                    <Typography variant='body1' color='text.secondary' display='flex' alignItems='center'>
                      <AttachMoney sx={{ mr: 1 }} /> Amount To Pay: ksh  <span style={{color: 'red', fontSize: '20px'}}> {specialistDetails.pricing} </span> 
                    </Typography>
                  </Paper>
                  ...
                  <Typography variant='body2' color='text.secondary' sx={{ pl: 2 }}>
                    - Make sure you enter the correct details including M-PESA number in the booking form.
                  </Typography>
                  <Typography variant='body2' color='text.secondary' sx={{ pl: 2 }}>
                    - After form submission, you will be prompted with an M-PESA Prompt  to enter your M-PESA Pin.
                  </Typography>
                  <Typography variant='body2' color='text.secondary' sx={{ pl: 2 }}>
                    - Upon receiving the payment, we will process it and allocate the requested appointment.
                  </Typography>
                  <Typography variant='body2' color='red' sx={{ pl: 2 }}>
                    - Note: When you cancel the M-PESA Prompt, your appointment will be automatically canceled.
                  </Typography>
                </Stack>
              </div>
            ) : null}
